        ...
        while req_nodes:
            ...
            if new_node is not None:
                ...
                if new_node is not None:
                    ...
                    raise RobotError("%s not found" % req_node,
                                     error_symbol=["NOT_EXIST", "BAD_NODE"])
            else:
                raise RobotError("%s not found." % req_node,
                                 error_symbol=["NOT_EXIST", "BAD_NODE"])
...
def get_simulate_file_info(path):
    ...
    if is_dir:
        raise RobotError("%s not file" % path,
                         error_symbol=["NOT_EXIST", "BAD_NODE"])
    ...
    if path.endswith(".fc"):
        ...
        if node["HEAD_TYPE"] == "EXTRUDER":
            if "FILAMENT_USED" not in node:
