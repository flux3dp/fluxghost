...
class BinaryUploadHelper:
    ...
    def feed(self, buf):
        ...
        if self.buffered < self.length:
            ...
            raise RuntimeError(
                'BAD_LENGTH' + ' recive too many binary data (should be %i but get %i' % (self.length, self.buffered),
                'recive too many binary data (should be %i but get %i' % (self.length, self.buffered),
            )
